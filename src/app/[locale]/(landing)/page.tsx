...
export default function Home(): React.JSX.Element {
	...
	return (
		<>
				...
				<NavigationMenu className="px-4 h-[10dvh] flex justif-around">
					<NavigationMenuList>
						<NavigationMenuItem>
							<NavigationMenuLink
								href="/"
								className={cn(navigationMenuTriggerStyle(), "font-bold text-2xl")}
							>
								{t("title")}
							</NavigationMenuLink>
						</NavigationMenuItem>
						...
						<NavigationMenuItem>
							<NavigationMenuLink href={APP_ROUTES.login} className={navigationMenuTriggerStyle()}>
								{t("login")}
							</NavigationMenuLink>
						</NavigationMenuItem>
			...
			<footer
				className={cn("flex flex-col items-center justify-center bg-secondary p-10", SPACING.SM)}
			>
	...
	return (
		...
		>
			{t.raw("features")?.map((feature: { key: string; title: string; description: string }) => (
				<Card
					className={cn("hover:shadow-md transition-all duration-200 break-words", {
				...
				>
					<CardHeader>
						<CardTitle className={cn("font-extrabold text-xl md:text-2xl font-old", SPACING.SM)}>
							<div>
								{feature.key === "0" && (
