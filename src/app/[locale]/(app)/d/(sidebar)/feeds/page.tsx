		...
		<>
			<section className={SPACING.SM}>
				<div className="flex gap-2">
					<h1 className="font-semibold tracking-tight text-3xl">{t("rssFeed")}</h1>
					<AddFeedForm />
				</div>
...
async function FeedsHeaderWrapper(): Promise<React.JSX.Element> {
	...
	return <FeedsHeader userFeedsWithContentsCount={userFeedsWithContentsCount} />
}

async function FeedsWrapper(): Promise<React.JSX.Element> {
	const [timeline, sess] = await Promise.all([dal.getUserFeedsTimeline(), dal.verifySession()])
