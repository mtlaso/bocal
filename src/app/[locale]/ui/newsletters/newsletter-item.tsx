        ...
        flex items-center gap-2"
			...
			<div className={SPACING.MD}>
				<div className={SPACING.SM}>
					<Label htmlFor={`url-${item.id}`} className="block text-sm font-medium">
						{t("descFeedUrl")}
					</Label>
					...
					<div className="flex gap-2">
						...
						<Button
							onClick={(): void => copyToClipboard(userfeedsfuncs.formatFeedURL(item.url))}
							variant="outline"
							size="icon"
						>
							<TbCopy />
				...
				<div className={SPACING.SM}>
					<Label htmlFor={`email-${item.id}`} className="block text-sm font-medium">
						{t("descUsermail")}
					</Label>
					...
					<div className="flex items-center gap-2">
						...
						/>
						<Button
							onClick={(): void => copyToClipboard(userfeedsfuncs.formatUsermail(item.eid))}
							variant="outline"
							size="icon"
						>
							<TbCopy />
		...
		<AlertDialog open={open} onOpenChange={onOpenChange}>
			<AlertDialogContent>
				<AlertDialogHeader>
					...
					<AlertDialogTitle>{t("deleteNewsletterContextMenu.title")}</AlertDialogTitle>
					<AlertDialogDescription>{t("deleteNewsletterContextMenu.desc")}</AlertDialogDescription>
				</AlertDialogHeader>
				<AlertDialogFooter>
					<AlertDialogCancel>{t("deleteNewsletterContextMenu.cancel")}</AlertDialogCancel>
					<AlertDialogAction
						disabled={isPending}
