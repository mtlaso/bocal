...
export default async function middleware(req: NextRequest): Promise<NextResponse> {
  ...
  */
	if (isProtectedRoute && !sessionCookie) {
		return NextResponse.redirect(new URL(`${langPrefix}${LOGIN_PATH}`, req.nextUrl.origin))
	}

	if (isPublicRoute && sessionCookie) {
		return NextResponse.redirect(new URL(`${langPrefix}${APP_ROUTES.links}`, req.nextUrl.origin))
	}
